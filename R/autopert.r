...
autopert <- function(netw_file_path,
    ...
    futile.logger::flog.appender(
                       futile.logger::appender.file(log_file), name = log_file)
    ## Capture all unflogged warnings and errors in flogger
    ## https://github.com/zatonovo/futile.logger/issues/36
    options(error = function() futile.logger::flog.warn(
                                                  geterrmessage(), name = log_file))
                                        # Starting timestamp
    ...
    spec <- import_spec(spec_path = spec_path,
                        loserum = loserum,
    ...
    stop_missing_nodes_perturbed(spec,
                                 missing_nodes_perturbed_overide,
                                 netw_variables,
                                 log_file)
    stop_missing_nodes_expected(spec,
    ...
    commands <- spec_commands %>%
        dplyr::mutate(command = paste0(spec_command)
    ...
    if (nosat == TRUE) {
        ...
        p <- progress::progress_bar$new(total = nrow(commands_short),
                                        force = TRUE,
                                        clear = FALSE,
                                        format =
    ...
    data_commands <- dplyr::full_join(commands,
                               data, by = c("file_name" = "filename")) %>%
        dplyr::select(-spec_command,
               ##-file_name_spec,
               -num_exp_perts,
               ##-source_bkgrd,
               ##-background_command,
    ...
    results <- dplyr::full_join(spec_levels,
        ...
        dplyr::mutate(mean_result = (hi + lo) / 2,
    ...
    if (nrow(missing_results) > 0) {
        flog.error(
            warning("Results missing, did you try and get an output without specifying any
    inputs?\n See:\n ",
    ...
    readr::write_csv(results_short,
                     file.path(results_dir, "results_short.csv"))
    readr::write_csv(results_score,
                     file.path(results_dir, "results_score.csv"))
    readr::write_csv(results_mismatch,
                     file.path(results_dir, "results_mismatch.csv"))
    readr::write_csv(results_short_node_summary,
    ...
    results_short_node_summary %>%
        ...
        ggplot2::labs(title = "Mismatch between modelled and expected results",
             y = "Difference \n (mean model result - expected)",
             x = "Measured Gene") +
        ggplot2::coord_flip() +
        ggplot2::theme_bw()
