...
combo_parallel <- function(netw_file_path,
    ...
    # Check for drug conflicts before starting parallel processing
    # This ensures errors are properly propagated
    if (!is.na(combo_drug_path) && drug_conflict_overide == FALSE) {
        if (!file.exists(combo_drug_path)) {
            stop("Drug file does not exist: ", combo_drug_path)
        }
        drugs <- import_drugs_clean(drug_path = combo_drug_path)
        check_drug_conflicts(drugs = drugs, node_col_name = node_col_name)
    }
