...
test_that("make_bkg_commands_combo handles nodes not in network", {
  ...
  # Should complete but with NA values for missing nodes
  result <- make_bkg_commands_combo(backgrounds, netw_variables)
  
  expect_equal(nrow(result), 1)
  expect_true(is.na(result$command_arg) || grepl("NA", result$command_arg))
})