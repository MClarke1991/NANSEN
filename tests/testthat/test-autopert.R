...
test_that("autopert integration test - Windows only", {
  ...
  log_content <- readLines(file.path(run_dir, "AutoPertLogs.log"))
  expect_true(any(grepl("Start", log_content)))
  expect_true(any(grepl("End", log_content)))
  expect_true(any(grepl("Running Autopert on model", log_content)))
